...
- Be honest about coupling. Do not use callbacks, registrations, or indirection patterns to hide a dependency that exists in practice for every caller — if all users of component A also use component B, A can simply depend on B. Abstraction only earns its keep when it enables real variation; otherwise it is complexity with no payoff.
- Keep public API minimal. If a symbol is only used within its own component, make it `static`. Never expose functions or types speculatively. Dead public surface area is worse than dead internal code because it implies contracts that no longer exist.
- Kconfig options must correspond to values that genuinely differ between projects or hardware configurations. Do not add options for hypothetical bench scenarios or values that every project leaves at the default — hardcode those and delete the option.
- Minimise code quantity. Fewer lines is almost always better — less to read, less to misunderstand, less to break. When something can be deleted, delete it. When something can be inlined without losing clarity, inline it. When a wrapper, helper, or layer adds no real value, remove it. Treat code volume as a cost, not a measure of effort.
