...
static void joystick_tx_task(void *arg) {
    ...
    for (;;) {
        ...
        can_joystick_t joy = { .y_pct = y_pct };
        (void)can_tx(CAN_ID_JOYSTICK, (const uint8_t *)&joy, sizeof(joy));

        status_ui_update("Joystick", "y=%u%%", (unsigned)y_pct);
...
esp_err_t joystick_init(void) {
    adc_unit_t y_u;
    esp_err_t err = adc_oneshot_io_to_channel(CONFIG_JOYSTICK_Y_GPIO, &y_u, &s_y_ch);
    ESP_RETURN_ON_ERROR(err, TAG, "Y GPIO %d not a valid ADC pin", CONFIG_JOYSTICK_Y_GPIO);

    const adc_oneshot_unit_init_cfg_t acfg = { .unit_id = y_u, .clk_src = ADC_DIGI_CLK_SRC_DEFAULT };
