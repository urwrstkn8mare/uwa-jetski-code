...
/* Latest joystick Y axis from the aux controller (0..100, 50 = centre). */
...
static void control_compute(int16_t height_cm,
    ...
    /* Joystick Y maps directly to a proportional pitch trim — full stick = full
     * pitch authority, up = +pitch. Deadzone near centre. */
    float joy_y_norm = 1.0f - ((float)joy_y_pct / 50.0f);
    if (fabsf(joy_y_norm) < 0.05f) joy_y_norm = 0.0f;
    if (joy_y_norm >  1.0f) joy_y_norm =  1.0f;
    if (joy_y_norm < -1.0f) joy_y_norm = -1.0f;
    float joy_pitch_trim = joy_y_norm * pitch_target_abs_max;
